...
// Keyboard driver: pull a queued XT key value from the host and fire INT 9.
// The host writes the full 16-bit value (make/break + modifiers + scancode)
// into mem[0x4A6], matching the SDL driver's encoding below.
#define HOST_KEYBOARD_DRIVER host_keyboard_poll((unsigned short *)&mem[0x4A6]) && (int8_asap = (mem[0x4A6] == 0x1B), pc_interrupt(7))
...
// Host-owned graphics state (8086tiny renders RGB332 pixels here).
unsigned short vid_addr_lookup[VIDEO_RAM_SIZE], cga_colors[4] = {0 /* Black */, 0x1F1F /* Cyan */, 0xE3E3 /* Magenta */, 0xFFFF /* White */};
unsigned char gfx8_frame[720 * 348];
char gfx_active = 0;

// PC speaker square-wave phase counter (advanced by the host audio callback).
unsigned int speaker_phase = 0;
...
// Expose the CGA text framebuffer (B800:0). The host overlays it over the
// ANSI terminal for software that renders text directly to video memory.
unsigned char *host_text_framebuffer(void) { return mem + 0xB8000; }

// PC speaker: produce one square-wave sample from the emulated PIT channel 2.
// Called by the host audio callback thread. Returns -1, 0 or 1.
int host_speaker_sample(void)
{
    unsigned char ctl = io_ports[0x61] & 3;
    unsigned short counter = CAST(unsigned short)mem[0x4AA];
    if (ctl == 3 && counter)
        return ((54 * speaker_phase++ / counter) & 1) ? 1 : -1;
    return 0;
}
...
int emulator_run(const char *bios_path, const char *floppy_path, const char *harddisk_path, const char *vdisk_dir)
{
	...
	lseek(disk[2], 0, 0);
	int bios_read = read(disk[2], regs8 + (reg_ip = 0x100), 0xFF00);
	(void)bios_read;
		...
		// Instruction execution loop. Terminates if CS:IP = 0:0 or the host
		// requests a stop/reset via host_control().
		for (;;)
		{
			...
			{
				int ctl = host_control();
				if (ctl == HOST_CTL_STOP)
					goto emulator_done;
				if (ctl == HOST_CTL_RESET)
					break;
			}
		...
		switch (xlat_opcode_id)
		{
			...
			OPCODE 22: // OUT DX/imm8, AL/AX
				...
				scratch_uint == 0x43 && (io_hi_lo = 0, regs8[REG_AL] >> 6 == 2); // Speaker/PIT mode command
		...
		// In text mode, mark the host frame dirty on each timer tick so the
		// renderer re-reads the B800:0 framebuffer (FreeDOS writes it directly).
		if (int8_asap && !(io_ports[0x3B8] & 2))
			host_text_dirty();
		...
		if (!(inst_counter % GRAPHICS_UPDATE_DELAY))
		{
			...
			if (io_ports[0x3B8] & 2)
			{
				// Compute colour and video memory translation tables once per mode entry
				if (!gfx_active)
				{
					for (int i = 0; i < 16; i++)
				...
				// Render the emulated graphics card video RAM into an RGB332 buffer.
				// The original wrote a 32-bit word per 4 pixels into an 8bpp SDL
				// surface; we expand that word into 4 RGB332 bytes here.
				vid_mem_base = mem + 0xB0000 + 0x8000*(mem[0x4AC] ? 1 : io_ports[0x3B8] >> 7); // B800:0 for CGA/Hercules bank 2, B000:0 for Hercules bank 1
				for (int i = 0; i < GRAPHICS_X * GRAPHICS_Y / 4; i++)
				{
					unsigned int px = pixel_colors[15 & (vid_mem_base[vid_addr_lookup[i]] >> 4*!(i & 1))];
					gfx8_frame[4*i + 0] = (unsigned char)(px & 0xFF);
					gfx8_frame[4*i + 1] = (unsigned char)((px >> 8) & 0xFF);
					gfx8_frame[4*i + 2] = (unsigned char)((px >> 16) & 0xFF);
					gfx8_frame[4*i + 3] = (unsigned char)((px >> 24) & 0xFF);
				}

				host_video_frame(gfx8_frame, GRAPHICS_X, GRAPHICS_Y);
			}
			else
			{
				gfx_active = 0;
		...
		if (int8_asap && !seg_override_en && !rep_override_en && regs8[FLAG_IF] && !regs8[FLAG_TF])
			pc_interrupt(0xA), int8_asap = 0, HOST_KEYBOARD_DRIVER;
	}
	}
